...
import { getAllItems as getProductos, getItemsByCategory } from '../../services/firebase/firebaseConfig';
...
function ItemListContainer({ greeting, items }) {
    ...
    useEffect(() => {
        if (categoryid === undefined) {
            document.title = 'EmmaLou Catalogo';
            getProductos().then(respuestaPromise => {
                setProductos(respuestaPromise);
                setLoading(false);
            });
        } else {
            document.title = `EmmaLou vivero! ${categoryid}`;
